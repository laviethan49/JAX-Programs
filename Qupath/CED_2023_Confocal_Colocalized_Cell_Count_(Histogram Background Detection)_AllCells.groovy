...
def GFPHisto = imagePixels.getHistogram(channelNames[1])//SP8 Channel 3 (index 2) is RFP DMi8 Channel 1 (index 0) is RFP
...
fileName = buildFilePath(folderName, labName+" "+animalNumber+'_Confocal_Analysis_AllCells.csv')
...
def peaksGFP = findPeaks(GFPHisto)
def peaksLengthGFP = peaksGFP.size()-1
def lastPeakGFP = peaksGFP[peaksLengthGFP]
positiveGFPThreshold = lastPeakGFP+(GFPHisto.getStdDev()*1)
...
def makeFile(data)
{
    ...
    if(!resultsFile.exists()) {
       resultsFile.append("Image Name, Background Threshold, DAPI Nuclei Count, tdTomato+ Neuron Count, tdTomato+ Percentage, ROI Area (um^2), Z-Stack Index, Pixel Accuracy for Cell Detection\n")
    }
...
//            possibleNeurons = detections.findAll {it.getMeasurements()['Cell: GFP mean'] > positiveGFPThreshold && it.getPathClass().toString().contains("Pos")}.size()
            ...
            roiArea = anno.getROI().getArea()*pixelScale*pixelScale
  
            deselectAll()      
            getCurrentHierarchy().getSelectionModel().setSelectedObject(anno, true)
            makeFile([positiveThreshold, totalNeurons, positiveCellCount, positivePercentage, roiArea, currentZ, pixelCalString])
            
            println(imageName)
            println("Background Intensity: "+positiveThreshold+", Total Cells: "+totalNeurons+", Positive Cells: "+positiveCellCount+", Percentage: "+positivePercentage+", Z-Stack: "+currentZ)
