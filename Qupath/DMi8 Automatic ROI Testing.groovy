        ...
        oldAnnotations.each {anno->
            ...
            if(annotationName != null) {
                ...
                if(anno.getChildObjects().size() > 0) {
                    ...
                    getCurrentHierarchy().getSelectionModel().setSelectedObject(expandedAnno, true)
                    expandedAnno.setName("RFP Pos Area of "+annotationName+" Expanded")
    
                    annoROI = expandedAnno.getROI()
                    ...
                    removeObject(expandedAnno, false)
                    expandedAnno = PathObjects.createAnnotationObject(currentROI)
                    expandedAnno.setName("RFP Pos Area of "+annotationName+" Expanded")
                    addObject(expandedAnno)
                    
                    annoArea = anno.getROI().getArea()*pixelArea
                    redArea = expandedAnno.getROI().getArea()*pixelArea
                    ...
                    anno.addChildObject(expandedAnno)
                    fireHierarchyUpdate()
                } else {
