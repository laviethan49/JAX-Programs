...
//
// To do:
//     - Add comment blocks to functions
//     - Add option to NOT overwrite previous runs
//     - write/read parameter
//     - directly access Omero server
//
var Version = "version 1.06";
var ImageTypes = newArray("liver", "kidney", "adrenalgland");
var ImageType = ImageTypes[0];
var Actions = newArray("areas", "counts");
var Action = Actions[0];
var OutputDirectory = "";
var bSaveValidation = true;
...
var vLowerThresholds = newArray(0, 0, 0);  // RFP, GFP, DAPI
...
//
// This mode processes images that are already open.  The user selects the image of interest
// from a list of open images.
//
...
//
// This mode processes all series in an lif or similar file.
// The BioFormats plugin is used to open the file.
//
function process_lif_file()
{
    ...
    Dialog.addNumber("DAPI lower threshold", vLowerThresholds[2]);
    Dialog.addCheckbox("use CLAHE", true);
    Dialog.addCheckbox("save validation files", bSaveValidation);
...
function process_image(InputImage)
{
    ...
    //
    // Split the channels and pre-process.
    // After this the channel images remain.
    //
    for (i=0; i<3; i++)
    {
        close(ChannelNames[i]);
    }
    split_channels(InputImage, bUseClahe);
    ...
    for (i=0; i<3; i++)
    {
        ...
        {
            //vCellCounts[i] = count_spots(ChannelNames[i]);  // jgp - old method
            count = stardist(ChannelNames[i]);
...
{
    if ( (bSaveValidation) && (OutputDirectory != File.separator()) )
    {
...
function split_channels(win, bUseClahe)
{
...
//
// Approximates the count of cells
//
...
//
// Parse the input filename for the animal sex, returned as a string.
// Valid return values are from the global array vSex.
//
...
//
// Parse the input file name for the tissue type.  The tissue type is returned as a string
// from the global array vTissue.
//
...
//
// Returns the number of cells as counted by the StarDist plugin
//
function stardist(win)
{
